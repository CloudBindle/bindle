...
# setup java_home
echo "JAVA_HOME=/usr/lib/jvm/java-7-oracle-cloudera" >> /etc/default/elasticsearch
...
perl -pi -e 's/\# discovery.zen.ping.timeout: 3s/discovery.zen.ping.timeout: 3s/' /etc/elasticsearch/elasticsearch.yml
# Only two nodes, it cannot find more than 1 master
